...
<section class="pt-32 pb-20 px-6 md:px-20 bg-forest min-h-screen">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-8 flex-wrap gap-4">
            <div>
                <h1 class="font-heading font-extrabold text-2xl md:text-3xl text-cream">Edit Foto Galeri</h1>
                <p class="text-sm text-cream/70">Perbarui informasi atau ganti foto galeri Desa Jiwut</p>
            </div>
        ...
        <div class="bg-cream rounded-2xl border border-cream/20 p-6">
            <form action="{{ route('admin.galeri.update', $galeri) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="bg-white rounded-2xl p-6 border border-brown/10 space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="kategori" value="{{ old('kategori', $galeri->kategori) }}"
                                   placeholder="mis. kegiatan, pemandangan, fasilitas"
                                   class="w-full rounded-lg border border-brown/20 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest/30">
                    ...
                    {{-- Gambar --}}
                    <div>
                        <label class="block text-sm text-brown/70 mb-2">Foto</label>

                        <div class="flex items-center gap-4 mb-3">
                            <img src="{{ asset('storage/'.$galeri->gambar) }}" alt="{{ $galeri->judul }}"
                                 class="w-32 h-24 object-cover rounded-lg border border-brown/10 flex-shrink-0">
                            <p class="text-xs text-brown/50">Foto saat ini. Unggah file baru di bawah kalau ingin menggantinya.</p>
                        </div>
                        ...
                        <p class="text-xs text-brown/50 mt-1">Format JPG/PNG/WEBP, maksimal 4MB. Kosongkan kalau tidak ingin mengganti foto.</p>
